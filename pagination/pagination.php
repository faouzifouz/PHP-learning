...
if ($_GET['page']) {
    $nbArticle = $_GET['page']*$nbpage+1;
    $page = $_GET['page'];
}
 
if ($page >= 1) {
    echo '<a href="pagination.php?page='.($page-1).'" class="precedent">Précédent</a> ';
}
 
if ($nbArticle+$page < count($contenu)) {
    echo ' <a href="pagination.php?page='.($page+1).'" class="suivant">Suivant</a>';
}
 
for ($i = 0; $i < $nbpage; $i++) {
    if ($contenu[$nbArticle+$i]) {
        echo '<div class="article">';
        echo '<h3>'.$contenu[$nbArticle+$i]['titre'].'</h3>';
        echo '<p>'.$contenu[$nbArticle+$i]['letexte'].'</p>';
        echo '<p>'.$contenu[$nbArticle+$i]['ladate'].'</p>';
        echo '</div>';
    }
}
?>
